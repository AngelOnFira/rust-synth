...
fn pro_kick() -> Box<dyn AudioUnit> {
    ...
    let sub = sine_hz::<f64>(25.0) * envelope(|t: f64| (-t / 0.8).exp()) * 0.2;
...
fn render_pattern(pattern: &str, instrument: Box<dyn AudioUnit>, bpm: f64, duration: f64) -> Vec<f32> {
    ...
    for (i, ch) in pattern.chars().cycle().enumerate() {
        let time = i as f64 * step_duration;
...
fn main() {
    ...
    let mut mix = vec![0.0f32; (duration * sample_rate as f64) as usize];
    ...
    // Normalize mix
    let max_val = mix.iter().fold(0.0, |max, &x| x.abs().max(max));
    if max_val > 0.0 {
